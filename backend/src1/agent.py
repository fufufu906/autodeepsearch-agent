...
from src1.models import SummaryStateOutput, SummaryState, TodoItem
from src1.prompts import report_writer_instructions, task_summarizer_instructions, todo_planner_instructions
...
network_retry_policy = retry(
    ...
    before_sleep=lambda retry_state: logger.warning(
        f"外部请求失败，正在进行第 {retry_state.attempt_number} 次重试..."
    )
)
...
class DeepResearchAgent:
    def __init__(self, config: Configuration | None = None) -> None:
        ...
        # ToolRegistry
        # 会把所有注册进来的工具（如搜索、笔记、日历）的描述信息汇总，生成一个巨大的
        # JSON
        # Schema
        # 验证参数格式对不对（比如
        # ID
    ...
    def _create_tool_aware_agent(self, *, name: str,
                                 system_prompt: str) -> ToolAwareSimpleAgent:
    ...
    def run(self,topic: str) ->SummaryStateOutput:
        ...
        if not  state.todo_items:
            logger.info("Planner failed to produce tasks, creating fallback task.")
    ...
    def run_stream(self, topic: str) -> Iterator[dict[str, Any]]:
        ...
        def worker(task: TodoItem, step: int) -> None:
            try:
                ...
                for event in self._execute_task(state,task,emit_stream=True):
                    enqueue(event, task=task)
            except Exception as e:
                logger.exception("Error executing task %d: %s", task.id, str(e))
                enqueue(
                    {
                        "types": "task_status",
                        "task_id": task.id,
                ...
                enqueue({"types": "__task_done__", "task_id": task.id})
        # # 为每个任务开辟一条线程，开启高并发检索
        for task in state.todo_items:
        ...
        try:
            while finished_workers < active_workers:
                ...
                if event.get("types") == "__task_done__":
                    finished_workers += 1
                    continue
                yield event# 关键：通过迭代器源源不断往外发射（喂给 FastAPI 的 StreamingResponse）
    ...
    # 给它一个具体的子任务（比如“搜集上海迪士尼的门票和攻略”），它负责把这个任务从头到尾执行完
    def _execute_task(
            self,
            state: SummaryState,
            task: TodoItem,
        ...
        try:
            # 使用带有重试机制的搜索
            search_result, notices, answer_text, backend = safe_dispatch_search()
        except Exception as e:
            logger.error(f"任务 {task.id} 搜索多次重试后彻底失败: {str(e)}")
            # 如果重试 3 次还是失败，优雅降级，当做没搜到处理
            search_result, notices, answer_text, backend = None, [f"搜索接口异常: {str(e)}"], "", "unknown"
        ...
        if emit_stream:
            ...
            # 对 LLM 生成阶段加上重试机制
            @network_retry_policy
            def safe_stream_summary():
                return self.summarizer.stream_task_summary(state, task, context)

            # 提前定义一个默认的兜底 getter，防止 try 块报错后 finally 找不到引用
            summary_getter = lambda: ""

            try:
                # 获取带有重试机制的流和真正的获取函数
                summary_stream, summary_getter_func = safe_stream_summary()
                summary_getter = summary_getter_func  # 替换为真实的 getter
                ...
                logger.error(f"任务 {task.id} LLM 生成重试彻底失败: {str(e)}")
                # 在前端打字机结尾追加一段错误提示，告诉用户部分内容生成失败
                yield {
                    "types": "task_summary_chunk",
                    "task_id": task.id,
                    "content": f"\n\n[AI 生成摘要时发生网络错误: {str(e)}]",
                    "step": step
                }
                summary_getter = lambda: f"生成失败: {str(e)}"  # 将失败信息作为最终保存结果
            finally:
                summary_text = summary_getter()
        else:
            # ：对非流式运行也加上重试防护
            @network_retry_policy
            def safe_summarize_task():
                return self.summarizer.summarize_task(state, task, context)

            try:
                summary_text = safe_summarize_task()
            except Exception as e:
                logger.error(f"任务 {task.id} LLM 非流式生成重试彻底失败: {str(e)}")
                summary_text = f"生成失败: {str(e)}"
    ...
    def _find_existing_report_note_id(self, state: SummaryState) -> str | None:
        ...
        for event in reversed(self._tool_tracker.as_dicts()):
            ...
            if not note_id:
                note_id = self._tool_tracker._extract_note_id(event.get("result", ""))  # types: ignore[attr-defined]
    ...
    def _extract_note_id_from_text(self, response: str) -> str | None:
        return self._tool_tracker._extract_note_id(response)  # types: ignore[attr-defined]
