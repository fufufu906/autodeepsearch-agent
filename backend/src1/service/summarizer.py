...
class SummarizationService:
    ...
    def summarize_task(self,state: SummaryState, task: TodoItem,context: str)->str:
        ...
        summary_text = strip_thinking_tokens(summary_text).strip()
    ...
    def stream_task_summary(self,state: SummaryState,
        ...
        emit_index = 0
        agent = self._agent_factory()
        # 在流式传输（Streaming）中，实时剔除大模型（如 DeepSeek-R1）的  标签以及里面包裹的“内心独白”（思维链文本），只把干净的、真正的答案输出给前端。
        ...
        # 控制着流式输出，还决定了要不要把大模型的原始数据流（Raw Stream）送去flush_visible() 过滤器进行“去思考标签”加工
